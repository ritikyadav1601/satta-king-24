...
export async function getGamesWithTodayResults() {
  ...
  const resultMap = new Map(rows.map((row) => [`${row.game}:${row.resultDate}`, row.result]));

  return games.map((game) => ({
    ...game,
    first: formatResult(resultMap.get(`${game._id}:${yesterday}`) || "XX"),
    second: formatResult(resultMap.get(`${game._id}:${today}`) || "XX")
  }));
}
